...
def get_project_list(bio_project_name,acc_list, verbose_print):
    with open(acc_list, 'r') as f:
        acc_list = f.read().splitlines()
        get_run_info_command = f'esearch -db sra -query {acc_list[0]} | efetch -format runinfo > sapir_run_info.csv'
        os.system(get_run_info_command)

        # Loop through the rest of the accession numbers and append the results
        for acc in acc_list[1:]:
            get_run_info_command = f'esearch -db sra -query {acc} | efetch -format runinfo >> sapir_run_info.csv'
            os.system(get_run_info_command)
    verbose_print(f"downloaded the run info at {bio_project_name}_run_info.csv")
...
def download(dataset_name, data_type, acc_list, verbose, specific_location):
    ...
    if acc_list_path is None:
        ...
        get_project_list(dataset_name,acc_list_path, verbose_print)
